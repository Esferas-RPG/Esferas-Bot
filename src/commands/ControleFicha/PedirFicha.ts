...
import { INewCharacterProps } from "../../interfaces/index.js";
...
class PedirFicha {
    ...
    async generate(interaction: ButtonInteraction) {
        try {
            ...
            this.data.registerId = user?.id; // Armazena o ID do recrutador em data
    ...
    async Form(interaction: ModalSubmitInteraction): Promise<void> {
        try {
            ...
            const apiRes = await api.newCharacterResponse(this.data);
