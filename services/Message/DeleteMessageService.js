export default async function deleteMessages(interaction, quantidade = 500) {
    ...
    try {
        while (deletedMessages < quantidade) {
            ...
            const deletableMessages = messages.filter((message) => new Date().getTime() - message.createdTimestamp <
                14 * 24 * 60 * 60 * 1000);
